...
public class VisualizarDenunciasAgentes extends AppCompatActivity {
    ...
    private ActivityVisualizarDenunciasAgentesBinding binding;
    private ExoPlayer exoPlayer;
    private AdapterVisualizarDenunciasAgentes midiasAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        binding = ActivityVisualizarDenunciasAgentesBinding.inflate(getLayoutInflater());
        ...
        if(!"Aguardando resposta".equals(status_reclamacao)){
            responder_reclamacao.setText("Visualizar resposta");
        }



        responder_reclamacao.setOnClickListener(v -> {

            Intent responder_denuncia = new Intent(VisualizarDenunciasAgentes.this, ResponderDenunciaActivity.class);
            responder_denuncia.putExtra("status_reclamacao", status_reclamacao);
            responder_denuncia.putExtra("ID", id);
            responder_denuncia.putExtra("UUID", uuid);
            startActivity(responder_denuncia);
        });
        ...
        setSupportActionBar(binding.toolbarVisualReclaAgentes);
    ...
    private void setupRecyclerView() {
        // CORRIGIDO: Use o RecyclerView correto (seu XML deve ter um RecyclerView com este ID)
        RecyclerView rvMidias = binding.rvMidias;  // ← Certifique-se que seu XML tem um RecyclerView com id "rvMidias"

        // Grid com 2 colunas
        GridLayoutManager gridLayoutManager = new GridLayoutManager(this, 2);
        rvMidias.setLayoutManager(gridLayoutManager);

        midiasAdapter = new AdapterVisualizarDenunciasAgentes();
        rvMidias.setAdapter(midiasAdapter);

        midiasAdapter.setOnMidiaClickListener(new AdapterVisualizarDenunciasAgentes.OnMidiaClickListener() {
            @Override
            public void onImagemClick(String url) {
                Log.d("CLICK", "Imagem clicada: " + url);
                Intent intent = new Intent(VisualizarDenunciasAgentes.this, FullscreenActivity.class);
            ...
            public void onVideoClick(String url) {
                ...
                Toast.makeText(VisualizarDenunciasAgentes.this, "Abrindo vídeo...", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(VisualizarDenunciasAgentes.this, FullScreenVisualizarMidiaReclamacoes.class);
                intent.putExtra("urlMidia", url);
                startActivity(intent);
            }
        });
    ...
    private void populaDados() {
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (!snapshot.exists()) {
                    Toast.makeText(VisualizarDenunciasAgentes.this,
                ...
                if (r != null) {
                    ...
                    binding.txtReferenciaEndReclamacao.setText(r.getReferencia());
                }
                ...
                if (midiaSnapshot.exists()) {
                    ...
                    if (!listaMidias.isEmpty()) {
                        midiasAdapter.setMidias(listaMidias);
                        binding.rvMidias.setVisibility(View.VISIBLE);
                    } else {
                        binding.rvMidias.setVisibility(View.GONE);
                    }
                } else {
                    binding.rvMidias.setVisibility(View.GONE);
                }
        ...
        Toolbar toolbar = binding.toolbarVisualReclaAgentes;
